package org.afterlike.lucid.gui.screen
...
import org.afterlike.lucid.gui.component.impl.*
...
class LucidGuiScreen : GuiScreen() {
    ...
    private val tabs: List<TabContent> = listOf(ChecksTab, AppearanceTab, SettingsTab)
    private var currentTabIndex = 0

    private val components = mutableListOf<BaseComponent>()
    private val tabButtons = mutableListOf<TabButtonComponent>()
    ...
    override fun initGui() {
        ...
        theme = ThemeUtil(ConfigHandler.messageColor)

        val centerX = width / 2
        val panelLeft = centerX - panelWidth / 2
        val panelTop = height / 2 - panelHeight / 2

        initCloseButton(panelLeft, panelTop)
        initTabButtons(panelLeft, panelTop)
        initContentButtons(panelLeft, panelTop)
        initScrollbar(panelLeft, panelTop)
        initLinkComponent(panelLeft, panelTop, centerX)
    }

    private fun initCloseButton(panelLeft: Int, panelTop: Int) {
    ...
    private fun initTabButtons(panelLeft: Int, panelTop: Int) {
        val tabWidth = panelWidth / tabs.size

        tabs.forEachIndexed { index, tab ->
            val tabButton = TabButtonComponent(
                index + 1,
                panelLeft + tabWidth * index,
            ...
            components.add(tabButton)
        }
    }

    private fun initContentButtons(panelLeft: Int, panelTop: Int) {
        val contentAreaHeight = panelHeight - headerHeight - tabHeight - contentPadding
        val tempContext = createTabContext(panelLeft, panelTop, hasScrollbar = false)

        contentHeight = tabs[currentTabIndex].initButtons(tempContext, mutableListOf())
        hasScrollbar = contentHeight > contentAreaHeight

        buttons.clear()
        val context = createTabContext(panelLeft, panelTop, hasScrollbar)
        contentHeight = tabs[currentTabIndex].initButtons(context, buttons)
    ...
    private fun initScrollbar(panelLeft: Int, panelTop: Int) {
    ...
    private fun initLinkComponent(panelLeft: Int, panelTop: Int, centerX: Int) {
    ...
    private fun createTabContext(panelLeft: Int, panelTop: Int, hasScrollbar: Boolean): TabContext {
        val effectiveWidth = getContentWidth(hasScrollbar)
        return TabContext(
            panelLeft = panelLeft,
            panelTop = panelTop,
            ...
            fullContentWidth = effectiveWidth,
            settingContentWidth = effectiveWidth - (44 + 8),
    ...
    private fun drawPanelBackground(panelLeft: Int, panelTop: Int) {
        drawRect(panelLeft, panelTop, panelLeft + panelWidth, panelTop + panelHeight, theme.backgroundColor.rgb)
        drawRect(
            panelLeft, panelTop + headerHeight + tabHeight, panelLeft + panelWidth,
    ...
    private fun drawPanelBorder(panelLeft: Int, panelTop: Int) {
        ...
        drawRect(
            panelLeft, panelTop + panelHeight - 1, panelLeft + panelWidth,
        ...
        drawRect(
            panelLeft + panelWidth - 1, panelTop, panelLeft + panelWidth,
            panelTop + panelHeight, theme.borderColor.rgb
        )
    }
    ...
    private fun drawHeader(panelLeft: Int, panelTop: Int, centerX: Int) {
        ...
        drawCenteredString(
            fontRendererObj, "§lLucid Configuration", centerX,
        ...
        buttons.forEach { button ->
            ...
            button.description?.let { description ->
                val wrappedDesc = GuiUtil.wrapText(
                    ...
                    else getContentWidth(hasScrollbar),
                    fontRendererObj
                )
    ...
    private fun drawAppearancePreview(panelLeft: Int, panelTop: Int) {
        ...
        drawRect(
            panelLeft, previewY, panelLeft + panelWidth,
        ...
        drawRect(
            panelLeft + panelWidth - 1, previewY, panelLeft + panelWidth,
            previewY + previewBoxHeight, theme.borderColor.rgb
        )
        drawRect(
            panelLeft, previewY + previewBoxHeight - 1, panelLeft + panelWidth,
            previewY + previewBoxHeight, theme.borderColor.rgb
        )
        drawRect(panelLeft, previewY, panelLeft + 1, previewY + previewBoxHeight, theme.borderColor.rgb)

        val vlText = if (ConfigHandler.showVLInFlag) " §7[VL:10]" else ""
        val wdrText = if (ConfigHandler.showWDR) " §c[WDR]" else ""

        fontRendererObj.drawStringWithShadow(
            ConfigHandler.getFormattedPrefix() + "§fPlayer §7failed §${ConfigHandler.messageColor}Check" + vlText + wdrText,
    ...
    private fun playButtonSound() {
        mc.soundHandler.playSound(PositionedSoundRecord.create(BUTTON_SOUND, 1.0f))
    }
    ...
    private fun handleButtonAction(button: ButtonComponent) {
        when (button.action) {
            ...
            ButtonComponent.ButtonAction.TOGGLE_SOUND -> {
                ConfigHandler.playSoundOnFlag = !ConfigHandler.playSoundOnFlag
                initGui()
            }

            ButtonComponent.ButtonAction.TOGGLE_VERBOSE -> {
                ConfigHandler.verboseMode = !ConfigHandler.verboseMode
                initGui()
            }
            ...
            ButtonComponent.ButtonAction.COLOR_PREV -> {
                val currentIndex = GuiUtil.colorCodes.indexOfFirst { it.first == ConfigHandler.messageColor }
                val newIndex = if (currentIndex <= 0) GuiUtil.colorCodes.size - 1 else currentIndex - 1
                ConfigHandler.messageColor = GuiUtil.colorCodes[newIndex].first
                initGui()
            }
            ...
            ButtonComponent.ButtonAction.TOGGLE_BOLD -> {
                ConfigHandler.messageBold = !ConfigHandler.messageBold
                initGui()
            }

            ButtonComponent.ButtonAction.SYMBOL_TOGGLE -> {
                ConfigHandler.messageSymbol = if (ConfigHandler.messageSymbol == ">") "»" else ">"
                initGui()
            }

            ButtonComponent.ButtonAction.TOGGLE_SHOW_VL -> {
                ConfigHandler.showVLInFlag = !ConfigHandler.showVLInFlag
                initGui()
            }

            ButtonComponent.ButtonAction.TOGGLE_SHOW_WDR -> {
                ConfigHandler.showWDR = !ConfigHandler.showWDR
                initGui()
            }
        }
    }
